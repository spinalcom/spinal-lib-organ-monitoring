...
export class ConfigFile {
	...
	public async init(connect: spinal.FileSystem, fileName: string, type: string, serverName: string, port: number): Promise<ConfigFileModel | undefined> {
		if (!fileName) {
			console.log("Monitoring file name was not provided, therefore this program will not be monitored");
			return;
		}
		return this._loadOrMakeConfigFile(connect, fileName, type, serverName, port).then((file) => {
			this.file = file;
	...
	private _loadOrMakeConfigFile(connect: spinal.FileSystem, fileName: string, type: string, serverName: string, port: number): Promise<ConfigFileModel> {
		return new Promise((resolve, reject) => {
			spinalCore.load(
				...
				() =>
					connect.load_or_make_dir("/etc/Organs/Monitoring", (directory: spinal.Directory) => {
						resolve(this._createFile(directory, fileName, type, serverName, port));
					}),
			);
		});
	}

	private _createFile(directory: spinal.Directory, fileName: string, type: string, serverName: string, port: number): ConfigFileModel {
